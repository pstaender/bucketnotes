...
export function Login({ setCredentials, errorMessage }) {
  const [loginCredentials, setLogiCredentials] = useState({
    ...
    secretAccessKey: sessionStorage.getItem("s3-secret-access-key") || undefined
  });
  ...
  return (
    <div className="login">
      <div className="headline">
        <img src={Logo} className="icon" alt="Bucketnotes" title="Bucketnotes"></img>
      </div>
                ...
                <input
                  ...
                  onChange={(ev) => {
                    if (ev.target.checked) {
                      if (
                        confirm(
                          "Secret will be stored in local storage in browser. Are you sure?"
                        )
                      ) {
